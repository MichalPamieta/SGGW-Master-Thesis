...
{
    public class ParallelDiffusionGA2D : IGeneticAlgorithm
    {
        ...
        protected GeneticAlgorithmResult RunAlgorithm(IProgress<double>? progress = null, CancellationToken cancellationToken = default)
        {
            ...
            for (int generation = 1; generation <= Parameters.MaxGenerations; generation++)
            {
                ...
                Parallel.For(0, rows, new ParallelOptions { MaxDegreeOfParallelism = Parameters.ThreadCount }, i =>
                {
                ...
                if (generation % Parameters.GeneticDriftFrequency == 0)
                {
                    ...
                    Parallel.For(0, drift.Length, new ParallelOptions { MaxDegreeOfParallelism = Parameters.ThreadCount }, i =>
                    {
                        drift[i] = nextGeneration[i].Clone();
                    });

                    Parallel.For(0, nextGeneration.Length, new ParallelOptions { MaxDegreeOfParallelism = Parameters.ThreadCount }, i =>
                    {

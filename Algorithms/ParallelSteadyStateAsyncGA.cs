...
{
    public class ParallelSteadyStateAsyncGA : IGeneticAlgorithm
    {
        ...
        protected async Task<GeneticAlgorithmResult> RunAsync(IProgress<double>? progress = null, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                int producerThreads = Math.Max(1, (int)(Parameters.ThreadCount * Parameters.ProducerRatio));
                int consumerThreads = Math.Max(1, Parameters.ThreadCount - producerThreads);

                population = PopulationHelper.InitializePopulationParallel(Parameters.PopulationSize, Parameters, TestFunction, Parameters.ThreadCount);
                await PopulationHelper.EvaluateFitnessParallelAsync(population, TestFunction, Parameters.ThreadCount, cancellationToken);

                FitnessHistory[0] = PopulationHelper.GetFitnessSummary(population);
                bestIndividual = PopulationHelper.GetBest(population);
                ...
                Task producer = Task.Run(async () =>
                {
                    while (producerEvaluations < maxEvaluations && !cancellationToken.IsCancellationRequested)
                    {
                        ...
                        Parallel.For(0, pairs, new ParallelOptions { MaxDegreeOfParallelism = producerThreads }, i =>
                        {
                            (Individual parent1, Individual parent2) = SelectionManager.SelectParents(population);
                ...
                {
                    await PopulationHelper.EvaluateFitnessParallelAsync(new Population(children), TestFunction, consumerThreads, cancellationToken);
            ...
            catch (OperationCanceledException)
            {
                ...
                return new GeneticAlgorithmResult(
                    ...
                    bestIndividual ?? new Individual(Parameters.GenotypeLength),
                    FitnessHistory,
                    timer.Elapsed,
                    timeToOptimum,
                    generationOfOptimum,
